...
public class DetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            checkIsFavorited(request, response);
            ...
            request.getRequestDispatcher(PageLink.DETAIL_HOME_PAGE).forward(request, response);
        } catch (Exception e) {
    ...
    private void checkIsFavorited(HttpServletRequest request, HttpServletResponse response) {
        int movieId = Integer.parseInt(request.getParameter("id"));
        HttpSession session = request.getSession(false);
        if (session != null && session.getAttribute(SessionUtil.USER_INFOR) != null) {
            List<Movie> favList = (List<Movie>) session.getAttribute(SessionUtil.FAV_LIST);
            boolean isFavorited = favList.stream().anyMatch(movie -> movie.getId() == movieId);
